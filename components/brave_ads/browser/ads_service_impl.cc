...
void AdsServiceImpl::MigratePrefs() {
  ...
  if (!PrefExists(prefs::kVersion)) {
    // Brave ads did not persist the default value for |prefs::kVersion| before
    // "https://github.com/brave/brave-browser/issues/5434" unless a migration
    // occurred, so we need to force migration of preferences from version |1|
...
bool AdsServiceImpl::IsUpgradingFromPreBraveAdsBuild() {
  ...
  // |prefs::kAdsPerDay| was set to |6| in 0.62.x and changed to |12| for mobile
  // and |20| for desktop in 0.63.x

  // |prefs::kVersion| was introduced in 0.63.x

  // So, we can detect if we are upgrading from a pre Brave ads build by
  // checking |prefs::kEnabled| is set to |true|, |prefs::kAdsPerDay| is set to
  // |6|, |prefs::kVersion| does not exist and it is not the first time the
  // browser has run for this user

  return GetBooleanPref(prefs::kEnabled) &&
      GetUint64Pref(prefs::kAdsPerDay) == 6 && !PrefExists(prefs::kVersion) &&
